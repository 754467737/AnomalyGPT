...
class AnomalyAnyCLIPDetector(nn.Module):
    def __init__(self, clip_model_name="ViT-L/14", clip_download_root="~/.cache/clip", image_size=224, selected_layers=None, prompt_templates=None, use_shared_adapter=True, debug=False):
        super().__init__()
        self.clip, _ = clip.load(clip_model_name, download_root=os.path.expanduser(clip_download_root), device="cpu", jit=False)
        self.clip.float().eval()
        self.image_size = image_size
        self.debug = debug
        self.prompt_templates = list(prompt_templates) if prompt_templates else ["a photo of a {} {}."]

        if selected_layers is None:
            selected_layers = [3, 6, 9, 12] if "B/16" in clip_model_name else [6, 12, 18, 24]
        ...
        vh = self.clip.visual.conv1.out_channels
        th = self.clip.text_projection.shape[1]
    ...
    def build_text_features(self, class_name: str, device: torch.device, defect_word: str = "anomalous") -> torch.Tensor:
        ...
        txt = clip.tokenize(normal + abnormal).to(device)
        with torch.no_grad():
            text_feats = F.normalize(self.clip.encode_text(txt), dim=-1)
    ...
    def _extract_vision(self, images):
        ...
        cls = self.clip.visual.class_embedding.to(x.dtype).unsqueeze(0).unsqueeze(0).expand(x.shape[0], -1, -1)
        ...
        patches = []
        for i, blk in enumerate(self.clip.visual.transformer.resblocks, start=1):
            x = blk(x)
        ...
        for images, _, labels, _, objs, _ in normal_loader:
            ...
            for j, obj in enumerate(objs):
                ...
                for i, (tok, _, _) in enumerate(patches):
                    per_obj[obj][i].append(self._adapt_patch(tok[j:j+1], i).reshape(-1, self.clip.text_projection.shape[1]))
    ...
    def forward(self, images: torch.Tensor, f_text: torch.Tensor, obj_names):
        ...
        s_vl = torch.softmax(torch.einsum("bc,bkc->bk", f_image, f_text), dim=-1)
        s_vl_abn = s_vl[:, 1]
        ...
        for i, (tok, ph, pw) in enumerate(patches):
            ...
            probs = torch.softmax(torch.einsum("bnc,bkc->bnk", adapted, f_text), dim=-1)[..., 1].reshape(b, 1, ph, pw)
            ...
            for bi, obj in enumerate(obj_names):
                ...
                assert bank is not None and bank.numel() > 0
        ...
        pixel = _minmax_norm(m_vl + m_vv)
        if self.debug:
            print(f"[DEBUG] F_image={f_image.shape} F_text={f_text.shape} M_VL={m_vl.shape} M_VV={m_vv.shape}")
        return {"image_score": s_vl_abn + s_vv, "pixel_map": pixel, "S_VL": s_vl, "M_VL": m_vl, "M_VV": m_vv, "S_VV": s_vv}
